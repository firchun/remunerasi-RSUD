<?php
$title = "Perbaikan Konsultasi - " . $_ENV['APP_NAME'];
ob_start();
?>
...
$poliklinik = [];
$resPoli = mysqli_query($koneksi, "SELECT kd_poli, nm_poli FROM poliklinik WHERE status = '1' ORDER BY nm_poli ASC");
while ($row = mysqli_fetch_assoc($resPoli)) {
  $poliklinik[] = $row;
}

$tindakan_konsultasi = [];
$resTindakan = mysqli_query($koneksi, "SELECT kd_jenis_prw, nm_perawatan, total_byrdrpr FROM jns_perawatan WHERE kd_kategori = 'KP003' AND status = '1' ORDER BY nm_perawatan ASC");
while ($row = mysqli_fetch_assoc($resTindakan)) {
  $tindakan_konsultasi[] = $row;
}
...
<div class="bg-white rounded-2xl border border-green-700 p-3 mb-3">
  <!-- Tabs Navigation -->
  <div class="nav-tabs">
    <button class="nav-link active" data-target="#tab-rajal">Rawat Jalan</button>
  </div>
...
<div class="bg-white rounded-2xl border border-green-700 p-3 mb-3">
  <div style="margin-bottom: 15px;">
    <button id="btnInputKonsultasi" class="btn-cari" style="display: none; background: #047857;">+ Input
      Konsultasi</button>
  </div>
  <div class="tab-content">
    <div id="tab-rajal" class="tab-pane">
      <div style="overflow-x:auto;">
        <table id="tableRajal" class="display" style="width:100%; font-size:12px;">
          <thead>
            <tr>
...
<!-- Modal Input Konsultasi -->
<div class="modal-overlay" id="modalInputKonsultasi"
  ...
  <div class="modal-box"
    ...
    <div class="modal-header"
      ...
      <h3 style="margin:0; font-size:16px;">Input Konsultasi</h3>
      <button class="modal-close" id="btnCloseModalKonsul"
        style="background:none; border:none; color:#fff; font-size:20px; cursor:pointer;">&times;</button>
    </div>
    <div class="modal-body" style="padding:20px;">
      <div id="alertKonsulSuccess"
      ...
      <form id="formInputKonsultasi">
        <div style="margin-bottom:15px;">
          <label style="display:block; font-size:13px; font-weight:600; margin-bottom:5px;">Tindakan Konsultasi</label>
          <select name="kd_jenis_prw" id="kd_jenis_prw"
            ...
            <?php foreach ($tindakan_konsultasi as $tk): ?>
              <option value="<?= $tk['kd_jenis_prw'] ?>"><?= $tk['nm_perawatan'] ?> - Rp
                <?= number_format($tk['total_byrdrpr'], 0, ',', '.') ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <div style="margin-bottom:15px;">
        ...
        <p style="font-size:12px; color:#6b7280; font-style:italic;">* Dokter akan otomatis disesuaikan dengan dokter di
          registrasi pasien.</p>
      </form>
    </div>
    <div class="modal-footer"
      style="padding:14px 20px; border-top:1px solid #e5e7eb; display:flex; justify-content:flex-end; gap:10px;">
      <button id="btnBatalKonsul"
...
<script>
  $(document).ready(function () {
    ...
    const tableRajal = $('#tableRajal').DataTable({
      ...tableConfig,
      ajax: {
        url: window.BASE_URL + '/api/get_data_perbaikan_konsultasi.php',
        type: 'POST',
        data: function (d) {
          d.bulan = $('#bulan').val();
          d.tahun = $('#tahun').val();
      ...
      columns: [
        ...
        { data: 'nm_poli' },
        {
          data: 'status_lanjut', render: function (data) {
            return `<span style="background:#fef3c7; color:#d97706; padding:2px 8px; border-radius:4px; font-weight:bold;">${data}</span>`;
          }
        }
      ]
    });
    ...
    $('#btnFilter').click(function () {
      ...
      $('#btnInputKonsultasi').hide();
      tableRajal.ajax.reload();
    });
    ...
    // Modal logic
    $('#btnInputKonsultasi').click(function () {
      $('#alertKonsulSuccess, #alertKonsulError').hide();
      $('#modalInputKonsultasi').css('display', 'flex');
    });

    $('#btnCloseModalKonsul, #btnBatalKonsul').click(function () {
      $('#modalInputKonsultasi').css('display', 'none');
    });

    $('#btnSimpanKonsul').click(function () {
      const selectedNos = [];
      ...
      if (!kdJenis || !nip) {
        alert('Lengkapi form tindakan dan petugas');
        return;
      }

      const btn = $(this);
      btn.text('Menyimpan...').prop('disabled', true);
      $('#alertKonsulError, #alertKonsulSuccess').hide();

      $.ajax({
        url: window.BASE_URL + '/api/save_input_konsultasi.php',
        method: 'POST',
        data: {
        ...
        success: function (res) {
          if (res.success) {
            $('#alertKonsulSuccess').text('\u2713 ' + res.message).show();
            setTimeout(function () {
              $('#modalInputKonsultasi').css('display', 'none');
              $('#chkAll').prop('checked', false);
              $('#btnInputKonsultasi').hide();
            ...
            $('#alertKonsulError').text('Gagal: ' + res.message).show();
          }
        },
        error: function () {
          $('#alertKonsulError').text('Terjadi kesalahan jaringan.').show();
        },
        complete: function () {
