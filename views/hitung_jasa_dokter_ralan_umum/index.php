  ...
  <div id="content-pasien" class="overflow-x-auto">
    <table id="tabelJasa" class="display w-full">
      ...
      <tfoot class="bg-green-800 font-bold text-white">
        <tr>
          <th colspan="5" class="text-right px-2">TOTAL AKHIR :</th>
          </tr>
      </tfoot>
    </table>
  </div>
...
<script>
  ...
  $(document).ready(function () {
    ...
    tablePasien = $('#tabelJasa').DataTable({
      ...
      buttons: [{
        extend: 'excel',
        text: 'Export Excel',
        filename: 'hitung_jasa_dokter_ralan_per_pasien_umum'
      }
      ],
      lengthMenu: [
      ...
      deferLoading: 0,
      ajax: {
        url: window.BASE_URL + '/api/get_data_hitung_jasa_dokter_ralan_umum.php',
        type: 'POST',
  ...
  function buatTabelRekap(withAjax) {
    var opts = {
      ...
      columns: [
        ...
        { data: 'total_tindakan_dr', className: 'num', render: function (v) { return v ? Math.round(v).toLocaleString('id-ID') : '0'; } }
      ],
      order: [[1, 'asc']],
      language: {
      ...
      footerCallback: function (row, data, start, end, display) {
        ...
        $(api.column(5).footer()).html(Math.round(sumData('jumlah_konsul')).toLocaleString('id-ID'));
        $(api.column(6).footer()).html(Math.round(sumData('jumlah_tindakan_lain')).toLocaleString('id-ID'));
        $(api.column(7).footer()).html(fmt(sumData('total_bpjs')));
        $(api.column(8).footer()).html(fmt(sumData('kolom_44')));
        $(api.column(9).footer()).html(fmt(sumData('total_tindakan_dr')));
        let sum_44 = sumData('kolom_44');
        let sum_jml_dpjp = sumData('jumlah_dpjp');
        let avg_pct = sum_44 > 0 ? ((sum_jml_dpjp / sum_44) * 100).toFixed(2) : 0;
        $(api.column(10).footer()).html(avg_pct + '%');
        $(api.column(11).footer()).html(fmt(sum_jml_dpjp));
      }
    };
    if (withAjax) {
      opts.ajax = {
        url: window.BASE_URL + '/api/get_rekap_hitung_jasa_dokter_ralan_umum.php',
        type: 'POST',
  ...
  function exportExcelPerDokter() {
    ...
    window.open(window.BASE_URL + '/api/export_hitung_jasa_dokter_ralan_umum.php?' + $.param(params), '_blank');
  }
