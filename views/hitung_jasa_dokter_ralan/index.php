...
$pageTitle = 'Hitung Jasa Dokter Rawat Jalan - RSUD MERAUKE';
    ...
    <table id="tabelJasa" class="display w-full">
      <thead class="bg-green-800 text-white">
        <tr>
          ...
          <th class="px-2 text-right" style="background:#166534;color:#fff">Konsul</th>
          <th class="px-2 text-right" style="background:#166534;color:#fff">Tindakan Lain</th>
      ...
      <tfoot class="bg-green-800 font-bold text-white">
        <tr>
          <th colspan="6" class="text-right px-2">TOTAL AKHIR :</th>
    ...
    <table id="tabelRekap" class="display w-full">
      <thead class="bg-green-800 text-white">
        <tr>
          ...
          <th class="px-2 text-right">Konsul</th>
          <th class="px-2 text-right">Tindakan Lain</th>
          <th class="px-2 text-right" style="background:#166534;color:#fff">Total BPJS</th>
          <th class="px-2 text-right" style="background:#166534;color:#fff">44%</th>
...
<script>
  ...
  $(document).ready(function () {
    ...
    tablePasien = $('#tabelJasa').DataTable({
      ...
      buttons: [{
        extend: 'excel',
        text: 'Export Excel',
        filename: 'hitung_jasa_dokter_ralan_per_pasien'
      }
      ],
      lengthMenu: [
      ...
      deferLoading: 0,
      ajax: {
        url: window.BASE_URL + '/api/get_data_hitung_jasa_dokter_ralan.php',
        type: 'POST',
      ...
      footerCallback: function (row, data, start, end, display) {
        ...
        $(api.column(3).footer()).html(fmt(sumData('nominal_rs')));
        $(api.column(4).footer()).html(fmt(sumData('total_bpjs')));
        $(api.column(5).footer()).html(fmt(sumData('kolom_44')));
        $(api.column(6).footer()).html(Math.round(sumData('jumlah_konsul')).toLocaleString('id-ID'));
        $(api.column(7).footer()).html(Math.round(sumData('jumlah_tindakan_lain')).toLocaleString('id-ID'));
        $(api.column(13).footer()).html(fmt(sumData('total_tindakan_dr')));
        $(api.column(14).footer()).html(fmt(sumData('jumlah_dpjp')));
      }
    });
  ...
  function buatTabelRekap(withAjax) {
    var opts = {
      ...
      scrollCollapse: true,
      dom: '<"flex justify-between items-center mb-4"lB>rtip',
      buttons: [{ extend: 'excel', text: 'Export Excel', filename: 'rekap_jasa_dokter_ralan' }],
      ...
      columns: [
        ...
        { data: 'jumlah_pasien', className: 'num' },
        { data: 'jumlah_pasien_bpjs', className: 'num' },
        { data: 'jumlah_pasien_non_bpjs', className: 'num' },
        { data: 'jumlah_pasien_klaim_bpjs', className: 'num' },
        { data: 'jumlah_konsul', className: 'num' },
        { data: 'jumlah_tindakan_lain', className: 'num' },
        ...
        { data: 'persen_dokter', className: 'num', render: function (v) { return v + '%'; } },
        { data: 'jumlah_dpjp', className: 'num font-bold', createdCell: function (td) { $(td).css('background', '#fefce8'); }, render: function (v) { return v ? Math.round(v).toLocaleString('id-ID') : '0'; } }
      ],
      order: [[1, 'asc']],
      language: {
      ...
      footerCallback: function (row, data, start, end, display) {
        ...
        $(api.column(3).footer()).html(Math.round(sumData('jumlah_pasien_bpjs')).toLocaleString('id-ID'));
        $(api.column(4).footer()).html(Math.round(sumData('jumlah_pasien_non_bpjs')).toLocaleString('id-ID'));
        $(api.column(5).footer()).html(Math.round(sumData('jumlah_pasien_klaim_bpjs')).toLocaleString('id-ID'));
        $(api.column(6).footer()).html(Math.round(sumData('jumlah_konsul')).toLocaleString('id-ID'));
        $(api.column(7).footer()).html(Math.round(sumData('jumlah_tindakan_lain')).toLocaleString('id-ID'));
        $(api.column(8).footer()).html(fmt(sumData('total_bpjs')));
        $(api.column(9).footer()).html(fmt(sumData('kolom_44')));
        ...
        $(api.column(11).footer()).html(avg_pct + '%');
        $(api.column(12).footer()).html(fmt(sum_jml_dpjp));
      }
    };
    if (withAjax) {
      opts.ajax = {
        url: window.BASE_URL + '/api/get_rekap_hitung_jasa_dokter_ralan.php',
        type: 'POST',
  ...
  function exportExcelPerDokter() {
    ...
    window.open(window.BASE_URL + '/api/export_hitung_jasa_dokter_ralan.php?' + $.param(params), '_blank');
  }
  ...
  function bukaModalInvoice(d) {
    ...
    var items = [
      ...
      { label: 'Jumlah Konsul', value: Math.round(d.jumlah_konsul).toLocaleString('id-ID') },
      { label: 'Tindakan Lain', value: Math.round(d.jumlah_tindakan_lain).toLocaleString('id-ID') },
      { label: '', value: '', divider: true },
    ...
    html += '<button onclick="window.open(\'' + window.BASE_URL + '/hitung-jasa-dokter-ralan/detail?kd_dokter=' + encodeURIComponent(d.kd_dokter) + '&bulan=' + $('#filter_bulan').val() + '&tahun=' + $('#filter_tahun').val() + '\',\'_blank\')" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition text-sm">Buka Detail Jasa</button>';
    html += '</div>';
    html += '</div>';
