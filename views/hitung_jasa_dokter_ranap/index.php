...
<script>
  ...
  $(document).ready(function () {
    ...
    tablePasien = $('#tabelJasa').DataTable({
      ...
      buttons: [{
        extend: 'excel',
        text: 'Export Excel',
        filename: 'hitung_jasa_dokter_ranap_per_pasien'
      }
      ],
      lengthMenu: [
      ...
      deferLoading: 0,
      ajax: {
        url: window.BASE_URL + '/api/get_data_hitung_jasa_dokter_ranap.php',
        type: 'POST',
      ...
      footerCallback: function (row, data, start, end, display) {
        ...
        $(api.column(12).footer()).html(fmt(sumData('jumlah_dpjp')));
      }
    });
  ...
  function buatTabelRekap(withAjax) {
    var opts = {
      ...
      footerCallback: function (row, data, start, end, display) {
        ...
        $(api.column(6).footer()).html(fmt(sumData('total_bpjs')));
        $(api.column(7).footer()).html(fmt(sumData('kolom_44')));
        $(api.column(8).footer()).html(fmt(sumData('total_tindakan_dr')));
        let sum_44 = sumData('kolom_44');
        let sum_jml_dpjp = sumData('jumlah_dpjp');
        let avg_pct = sum_44 > 0 ? ((sum_jml_dpjp / sum_44) * 100).toFixed(2) : 0;
        $(api.column(9).footer()).html(avg_pct + '%');
        $(api.column(10).footer()).html(fmt(sum_jml_dpjp));
      }
    };
    if (withAjax) {
      opts.ajax = {
        url: window.BASE_URL + '/api/get_rekap_hitung_jasa_dokter_ranap.php',
        type: 'POST',
  ...
  function exportExcelPerDokter() {
    ...
    window.open(window.BASE_URL + '/api/export_hitung_jasa_dokter_ranap.php?' + $.param(params), '_blank');
  }
