...
$q_tarif = "SELECT material, bhp, tarif_tindakandr, tarif_tindakanpr, kso, menejemen, total_byrdrpr 
            FROM jns_perawatan_inap WHERE kd_jenis_prw = '$kd_jenis_prw' AND kd_kategori = 'KP026'";
$res_tarif = mysqli_query($koneksi, $q_tarif);

if (!$res_tarif || mysqli_num_rows($res_tarif) == 0) {
    echo json_encode(['success' => false, 'message' => 'Tindakan visite tidak ditemukan']);
    exit;
}
...
if ($success_count > 0) {
    ...
    echo json_encode(['success' => true, 'message' => "Berhasil menyimpan $success_count data visite"]);
} else {
